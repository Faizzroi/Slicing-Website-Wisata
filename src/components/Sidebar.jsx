...
const Sidebar = () => {
  ...
  const activated = ({ isActive }) => {
    return (
      "inline-block w-full h-auto flex justify-center " +
      (isActive ? "text-blue-600" : null)
    );
  };
    ...
    <>
      ...
      <nav className="md:hidden h-[68px] w-full shadow-[0px_1px_4px_2px_#00000070] bg-white z-10 fixed flex items-start justify-start">
        <div className="w-full flex items-center justify-between h-full">
          <div className="w-auto h-full flex">
            {path.search("/detail" || "/perbarui") !== 0 ? (
              <div className="text-[45px] ml-2 hover:cursor-pointer h-full flex items-center">
                <Link to="#side-bar">
                  <HiOutlineMenuAlt2 />
                </Link>
              </div>
              ...
              <NavLink
                ...
                className="h-full flex items-center min-w-max"
              >
                <div
                  className="text-[50px] pr-1 ml-1 hover:cursor-pointer"
                  hidden={path.search("/detail" || "/perbarui") !== 0}
                >
          ...
          <h1
            id="makan"
            className="text-black font-bold max-md:text-[28px] mr-4"
            hidden={window.location.hash == "#side-bar"}
          >
            Hi, {userName}!
          </h1>
        </div>
        {currentHash == "#side-bar" && (
          <div
            className={
              "z-30 fixed left-0 top-1/2 -translate-y-1/2 h-screen w-screen bg-black opacity-60"
            }
            onClick={() => history.back()}
          ></div>
        )}
        ...
        <div
          id="side-bar"
          className={
            "z-50 fixed left-0 top-1/2 -translate-y-1/2 h-[100%] duration-200 w-[65%] bg-white flex flex-col justify-between " +
            (currentHash == "#side-bar" ? null : "-translate-x-full")
          }
        >
          <div className="flex flex-col w-[full] h-[89.8%] gap-3 items-center text-center">
            ...
            <NavLink to={"/home"} className={activated}>
              <div
                id="toHome"
                className="text-[11vw] hover:cursor-pointer flex items-center justify-start hover:bg-gray-200 duration-150 rounded-xl w-[93%] py-[4px]"
              >
            ...
            <NavLink className={activated} to={"/table"}>
              <div
                className="text-[9vw] hover:cursor-pointer flex justify-start duration-150 rounded-xl p-2 py-[6px] md:max-lg:pl-[2px] pl-[1px] hover:bg-gray-200 w-[93%]"
                id="toTable"
              >
          ...
          <div className="flex flex-col items-center h-[18vw] gap-2">
            <hr className="bg-black min-h-[3px] w-[88%] right-0" />
            <div
              className="text-[10vw] hover:cursor-pointer flex justify-center duration-150 rounded-xl p-2 py-[1vw] md:max-lg:pl-[2px] pl-[1px] hover:bg-gray-200 w-[93%]"
              id="toTable"
              onClick={popup}
            >
